...
.intrinsicValue <- function(delPrices, adjRecPrices, commodCharge)
{
	...
	return(unlist(lapply(delPrices - adjRecPrices - commodCharge, max, 0)))
}
									...
									winDemand, sumCommod, winCommod, dailyOrMonthly, monthlyOrDaily)
{
	...
	data$Fuel_Adjusted_Receipt_Price <- round(.adjReceiptPrice(data$Receipt_Forward_Price, data$Fuel_Charge, country), 3)
	...
	data$Put_Value <- .putValue(data$F, data$d1, data$d2, data$Risk_Free_Rate, data$Time_to_Maturity, 
			data$Fuel_Adjusted_Receipt_Price, data$Commodity_Charge)
	data$Intrinsic_Value <- .intrinsicValue(data$Delivery_Forward_Price, data$Fuel_Adjusted_Receipt_Price, data$Commodity_Charge)
...
TransportSpreadOption$yearlyData <- function(rawData, startStrip, endStrip)
{
	...
	data[5, c(2:length)] <- -1 * as.vector(unlist(lapply(names(data)[-1], function(x){return(sum(rawData$NPV_Demand[rawData$Year==x]))})))
...
TransportSpreadOption$totalData <- function(rawData)
{
	...
	data[["Demand"]] <- -1 * sum(rawData$NPV_Demand)
	...
	data[[1]] <- dollar(data[[1]])
	data[[2]] <- dollar(data[[2]])
	data[[3]] <- dollar(data[[3]])
	data[[4]] <- dollar(data[[4]])
	data[[5]] <- dollar(data[[5]])
	data[[6]] <- dollar(data[[6]], do.dollar=FALSE)
	data[[7]] <- dollar(data[[7]], do.dollar=FALSE)
	return(data)
}
