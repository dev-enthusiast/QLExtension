...
RLoggerLevels$.logLevel <- 3  #RLoggerLevels$LOG_MESSAGE

# Private variable used to store the current output connection
RLoggerLevels$.logConnection <- ""
...
{
  RLoggerLevels$.logConnection <- connection
}
...
{
  if( RLoggerLevels$.logLevel >= logLevel )
  {
    cat(prefix, ..., "\n", file = RLoggerLevels$.logConnection )
  } 
}
