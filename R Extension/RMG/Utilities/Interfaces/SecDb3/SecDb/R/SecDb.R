...
.onLoad <- function(lib, pkg) {
    packageStartupMessage("Loading RSecDbInterface... ", appendLF=FALSE)
    
    options(warn.FPU=FALSE)
    dyn.load(Sys.which("RSecDbInterface.dll"))
    options(warn.FPU=TRUE)
    # maybe I could add the contents to .Rprofile
    source("S:/All/Risk/Software/R/prod/Energy/Trading/Congestion/lib.SecDb.core.R")
...
secdb.getValueType <- function( securityName, valueTypeName, debug=FALSE )
{
    ...
    returnValue = .SDB$get_value_type(security, valueType)
...
secdb.evaluateSlang <- function( slangCode, debug=FALSE )
{
    ...
    returnValue = .SDB$evaluate_slang( slang )
...
secdb.invoke <- function( libName, funcName, ... )
{
    ...
    returnValue = .SDB$call_function(libraryName, functionName, arguments )
...
# 
#
secdb.dateRuleApply <- function( dateValue, ruleToApply, calendar=NULL )
{
    dateFormat = "%d%b%y"
...
tsdb.getSymbolInfo <- function( curveName, debug=FALSE )
{
    ...
    returnValue = .SDB$get_symbol_info(curve)
