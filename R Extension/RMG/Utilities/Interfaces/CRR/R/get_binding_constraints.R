get_binding_constraints <- function(region="NEPOOL", type="DA",
   startDt=NULL, endDt=NULL, constraintNames=NULL)
{
  if (!is.null(startDt))
  ...
  if (region=="NEPOOL"){
    BC <- .getBindingConstraints.NEPOOL(type, startDt, endDt, constraintNames)
  }
  if (region=="NYPP"){
    BC <- .getBindingConstraints.NYPP(type, startDt, endDt, constraintNames)
  }
  BC <- unique(BC)
  BC <- BC[!is.na(BC$time),]   # there are some NA's in the time column!
  BC <- BC[order(BC$time, BC$constraint.name),]
...
{
  if (length(setdiff(toupper(type), c("DA","RT"))) != 0)
    stop("Type needs to be DA, RT, or both.") 
  

  # this DB ends in 2009-05
  con.string <- paste("FileDSN=",
       fix_fileDSN("S:/All/Risk/Software/R/prod/Utilities/DSN/ISOP.dsn"),
       ";UID=congestion_read;PWD=congestion_read;", sep="")  
  con <- odbcDriverConnect(con.string)
  on.exit(odbcCloseAll())
  
  query <- paste("SELECT constraintname, period, contingency, ptid, shadowprice, type ", 
       "FROM congstn.trans_constraints ",
       "WHERE iso = 'NYISO' ",
       "AND type in ('", paste(type, collapse="', '"), "') ", sep="")

  add <- "and"
  if (!is.null(start.dt)){
    query <- paste(query, add, " period >= to_date('", start.dt,
      "', 'yyyy-mm-dd') ", sep="")
    add   <- "and"
  }
  if (!is.null(end.dt)){
    query <- paste(query, add, " period <= to_date('", end.dt,
      "', 'yyyy-mm-dd') ", sep="")
    add   <- "and"
  }
  if (!is.null(constraint.names)){
    aux <- paste(constraint.names, sep="", collapse="', '")
    query <- paste(query, add, " constraintname in ('", 
      aux, "') ", sep="")
    add   <- "and"
  }
  ...
  names(BC) <- gsub("_", ".", tolower(names(BC)))
#browser()  
  BC$time <- BC$period + 3600  # change to Hour Ending
  
  BC <- BC[, c("constraintname", "contingency", "time", "shadowprice", "ptid", "type")]
