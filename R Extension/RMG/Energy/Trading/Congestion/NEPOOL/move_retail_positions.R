...
# 
.retailPositionsMove <- function(QQ, asOfDate, threshold=10)
{
  ...
  aux$roundMW <- -10*(aux$mw %/% 10 + 1)   # flip the sign
  ...
  qq$`HUB_2X16H` <- -apply(qq[,ind], 1, sum)
  ind <- grep("_5X16$", names(qq))
  qq$`HUB_5X16`  <- -apply(qq[,ind], 1, sum)
  ind <- grep("_7X8$", names(qq))
  qq$`HUB_7X8`   <- -apply(qq[,ind], 1, sum)
...
QQQ <- .retailPositionsMove(QQ, asOfDate, threshold=10)

startDate <- QQQ$month[1]
endDate   <- QQQ$month[nrow(QQQ)]
PP <- .getPrices(asOfDate, startDate, endDate)
