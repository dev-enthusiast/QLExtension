...
#  browser()
  valMonths <- sort(unique(as.Date(DataTable1$VALUATION_MONTH)))
  ...
  Rem08 <- which(valMonths > valMonths[1] & valMonths < as.Date("2009-01-01"))
  ...
  Groups <- list(1, 2, c(2,3), Sum07, Sep07, Fal07, Win08, Rem08, Cal07, Cal08, Cal09, All)
...
#  browser()
  DataTable <- DataTable[which(as.Date(DataTable$VALUATION_MONTH) %in% classFactor$timeGroup),]
