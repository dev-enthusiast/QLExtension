...
mainPortfolioReportUpstreamExcludeCEP <- function(options) {
  ...
  for ( i in 1:length(options$portfolio) ) {
    for(j in 1:length(options$dates)) {
      ...
      fileName     <- paste(options$dirOut, "Positions ", options$portfolio[i], " ",
                                    as.character(options$asOfDate), "_UpstreamExcludeCEP.xls", sep = "")  
                                                                             
          
        books <- as.vector(get.upstream.books()[,1])
        cepBooks <- as.vector(get.portfolio.hierarchy("CEP Overall Portfolio", options$asOfDate))
        options$books <- setdiff(books, cepBooks)
