...
mainPortfolioReportExcludeCEP <- function(options) {
  ...
  for ( i in 1:length(options$portfolio) ) {
    for(j in 1:length(options$dates)) {
      ...
      fileName     <- paste(options$dirOut, "Positions ", options$portfolio[i], " ",
                                    as.character(options$asOfDate), "_ExcludeCEP.xls", sep = "")  
      fileNamePos  <- paste(options$dirOut, "Positions ", options$portfolio[i], " ",
                                    as.character(options$asOfDate), "POS_ExcludeCEP.xls", sep = "")                                                                              
          
        books <- as.vector(get.pm.books()[,1])
        upstream <- as.vector(get.portfolio.hierarchy("Upstream Portfolio", options$asOfDate))
        upstreamPM <- as.vector(get.portfolio.hierarchy("Upstream PM Trading", options$asOfDate))
        options$books <- setdiff(books, upstream)
        options$books <- setdiff(options$books, upstreamPM)
...
#  browser()
  ...
  MMM1 <- data.frame(MMM1, FLOW_TYPE = "Total")
  colnames(MMM1) <- c("ECONOMIC_OWNER","COMMODITY","REGION","GREEK", "variable", "TIME_FRAME", "value", "FLOW_TYPE")
  formula <- paste(paste(colnames(MMM)[-which(colnames(MMM) %in% c("value", "REGION"))], 
                         collapse = " + ", sep = ""), " ~ .", sep = "")
  MMM2 <- cast(MMM, eval(formula), sum)  
  MMM2 <- data.frame(MMM2, REGION = "Total")
  colnames(MMM2) <- c("ECONOMIC_OWNER","COMMODITY","FLOW_TYPE","GREEK", "variable", "TIME_FRAME", "value", "REGION")                       
  formula <- paste(paste(colnames(MMM)[-which(colnames(MMM) %in% c("REGION", "value", "FLOW_TYPE"))], 
                         collapse = " + ", sep = ""), " ~ .", sep = "")
  MMM3 <- cast(MMM, eval(formula), sum) 
  MMM3 <- data.frame(MMM3, REGION = "Total", FLOW_TYPE = "Total")
  colnames(MMM3) <- c("ECONOMIC_OWNER","COMMODITY","GREEK", "variable", "TIME_FRAME", "value","REGION", "FLOW_TYPE")                  
  MMM <- rbind(MMM, MMM1)  
  MMM <- rbind(MMM, MMM2)   
