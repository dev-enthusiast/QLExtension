...
mainPortfolioReportE_uat <- function(options) {
  ...
  source("H:/user/R/RMG/Utilities/bookMappingCPSUAT.R")  
  ...
  for ( i in 1:length(options$portfolio) ) {
    for(j in 1:length(options$dates)) {
      ...
      options$dirOut <- paste("//ceg/cershare/Risk/Portfolio Management/Reports/PositionVaRReports_NewBookMapping/", as.character(options$asOfDate), "_NewBookMapping/", sep = "")
      dir.create(options$dirOut,  showWarnings = FALSE)
# Rename the file name here if needs to rerun.
      fileName     <- paste(options$dirOut, "Positions ", options$portfolio[i], " ",
                                    as.character(options$asOfDate), "_D.xls", sep = "")  
      fileNamePos  <- paste(options$dirOut, "Positions ", options$portfolio[i], " ",
                                    as.character(options$asOfDate), "POS_D.xls", sep = "")                                                                              
      #if(file.exists(fileName) == FALSE) {
      #  options$books     <- get.portfolio.hierarchy(options$portfolio[i], options$asOfDate)
      #  options$booksX <- c("BBLCAP", "MISOANI", "MISOANI2", "COALMTM", "MTMPHYOP", "PJMANI", "PJMANI2", "PJMANI3", 
      #                      "PJMANI4", "PJMANI5", "PJMANI6", "USMLTMTM", "USOPTMTM")                           
      #  options$booksX <- toupper(c(options$booksX, 
      #    get.portfolio.hierarchy("Hydrocarbons Intl Acc Portfolio", options$asOfDate)))                           
      #  indX <- which(options$books %in% options$booksX) 
      #  if(length(indX) >0 ) { options$books <- options$books[-indX]}   
      #  options$booksXX <-  c("MDLGATOL", "MDLGECEN", "MDLGECLD", "MDLGTCCP",	"MDLGTCNG",	"MDLGTCPW",	"MDLGTCTR", "MDLJCKEN",
      #  	                    "MDLJCKLD", "MDLSONTR")	 
      #  indXX <- which(options$books %in% options$booksXX) 
      # if(length(indXX) >0 ) { options$books <- options$books[-indXX]}
        # exclude GTC Accrual Portfolio
#        booksExc <-  get.portfolio.hierarchy("GTC Accrual Portfolio", options$asOfDate)
#        options$books <- setdiff(options$books, intersect(booksExc, options$books))
        
#        browser()
#        Abhi's way to assemble Mark Orman Portfolio 
#        b1p <- get.portfolio.hierarchy("CPS Accrual Portfolio", options$asOfDate)  
#        b2p <- get.portfolio.hierarchy("CPS MTM Hedges of Accrual", options$asOfDate) 
#        bp <- union(b1p,b2p)
#        b1m <- get.portfolio.hierarchy("Transmission Hedges Portfolio", options$asOfDate)
#        b2m <- get.portfolio.hierarchy("PJM ANI Portfolio", options$asOfDate)
#        b3m <- get.portfolio.hierarchy("Miso ANI Portfolio", options$asOfDate)
#        b4m <- get.portfolio.hierarchy("Power Trading", options$asOfDate)
#        b5m <- get.portfolio.hierarchy("Ontario Accrual Portfolio", options$asOfDate)
#        b6m <- c("COLADJBS", "COLADJCR", "COLADJWG", "PJMANI", "PJMANI2", "PJMANI3", "PJMANI5", "PJMANI6", 	   
#                 "MISOANI", "MISOANI2") 
#        b7m <- c("MDLGATOL", "MDLGECEN", "MDLGECLD", "MDLGTCCP",	"MDLGTCNG",	"MDLGTCPW",	"MDLGTCTR", "MDLJCKEN",
#                	                    "MDLJCKLD", "MDLSONTR")       
#        bm <- union(union(union(b1m,b2m), union(b3m,b4m)), union(union(b5m,b6m),b7m))                     
#        bookAbhi <- setdiff(bp,bm)
        
#       Margo's list
#        <- read.csv('c:/PM/masterbooklist-040907.csv')
#        indPM <- which(xxx$ORGANIZATION == "PM")
#        xxxPM<- xxx[indPM,]
#        setdiff(options$books, xxxPM$BOOK)

#        books <- read.csv("c:/temp/bookAbhi.csv")
#        books1 <- toupper(unique(books$BOOK))
#        books <- read.csv("c:/temp/bookCap.csv")
#        books2 <- toupper(unique(books$BOOK))        
#        booksX <- unique(c(as.character(books1),as.character(books2)))

#       UK books: from Yaxing
#        browser()     
        # Debug
     #   cat("calling get.portfolio.hierarchyCCG \n")  
     #   b1 <- get.portfolio.hierarchyCCG("Freight Accrual Portfolio", options$asOfDate) 
     #   b2 <- get.portfolio.hierarchyCCG("Coal International Portfolio", options$asOfDate)
     #   b3 <- get.portfolio.hierarchyCCG("Coal Phys Econ Hdg Portfolio", options$asOfDate)       
     #   b4 <- get.portfolio.hierarchyCCG("Coal Physical MTM Portfolio", options$asOfDate)
     #   b5 <- get.portfolio.hierarchyCCG("Euro Power And Gas Portfolio", options$asOfDate)
     #   bAll <- unique(c(as.character(b1), as.character(b2), as.character(b3), as.character(b4), as.character(b5)))
        
        #options$books <- unique(c(bAll, options$books))     
        books <- get.pm.books()
        options$books <- as.character(books[,1])
        # Debug
...
#        browser()
        # Debug
        cat("calling  bookMappingCPSUAT \n")
        dataBookMapping <- bookMappingCPSUAT(options) # adds ECONOMIC_OWNER
        ...
        dataByRegion <- data
        #recondata <- aggregate(pwrData$DELTA, by=list(pwrData$COMMODITY, pwrData$BOOK, 
        #    pwrData$VALUATION_MONTH, pwrData$REGION, 
        #    pwrData$SUBREGION), sum)

#        browser()
#        books <- read.csv("C:/Temp/BookFP.csv")     # book subset
#        data1 <- subset(data, BOOK %in% books[[1]])  # book subset
#        data1 <- subset(data, BOOK %in% c("CNECT", "CNENEMA", "CNEWMA"))  # book subset
#        data <- rbind(data1, data2)
#        browser()   
        # indV <- which(as.character(dataX$VALUATION_MONTH) == "2007-12-01")
        # indV <- which(as.numeric(format(as.Date(dataX$VALUATION_MONTH), "%Y")) == 2009)
        # indC <- which(dataX$COMMODITY == "POWER
        # indF <- which( dataX$FLOW_TYPE == "ON-PEAK")
        # indR <- which(dataX$REGION %in% c("NOX"))
        # indB <- which(data$BOOK == "KENDALL")
        # indX <- intersect(intersect(indV,indC), intersect(indF,indR))
        # dataZ <- dataX[indX,]    
        # dataXX <- aggregate(dataX$DELTA, list( book = dataX$BOOK, region = dataX$REGION), sum)
        # dataZZ <- aggregate(dataZ$DELTA, list( valuation_month = dataZ$VALUATION_MONTH, region = dataZ$REGION), sum)         
        # xxx <- aggregate(xxx$DELTA, list(BOOK = xxx$BOOK), sum)
        # dataXXX <- subset(dataXX, EO == "Mike Cocco")
        # xxx <- read.csv('c:/temp/Book8.csv')
        # setdiff(dataXX$BOOK, xxx$BOOK)  
        # dataXX[which(dataXX$BOOK %in% setdiff(dataXX$BOOK, toupper(xxx$BOOK))),]
        #           
        # rename Cap
...
#        browser()
        ...
        for (k in 1:(length(res)/2)) {
          ZZZ <-  RRR(res[[(2*k-1)]])    
          head(ZZZ)
          YYY <-  as.character(res[[(2*k)]])
          cat(paste("**** ", YYY, " \n", sep=""))                                      
          write.xls(ZZZ, fileName, as.character(YYY), visible=FALSE)
        }      
      #}
    }
#    browser()
