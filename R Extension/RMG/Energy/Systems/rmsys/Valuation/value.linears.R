# Value all the linear instruments:
#    PHYFWD   - instrument.type.id=1
#    FINSWAP  - instrument.type.id=2
#    FUT      - instrument.type.id=3
#    DLYSWAP  - instrument.type.id=20
#    CLRSWAP  - instrument.type.id=22


value.linears <- function(vrp, dd, options){
  ...
  res <- aggregate(res$x, by=list(CONTRACT.DATE = res$CONTRACT.DATE,
               SHIFT = res$SHIFT), sum)
  colnames(res)[3] <- "MTM"
  res <- cbind(INSTRUMENT.TYPE=options$instrument.type.id, res)
  return(list(res,vrp))
}
