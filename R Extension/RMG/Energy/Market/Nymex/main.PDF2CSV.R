...
##  browser()
  ...
  if(file.exists(fileTXT) == FALSE){
    command <- paste("S:/Data/PDF/XPDF/pdftotext", filePathName, sep = " ")
    system(command)
  }
  ...
  ListAllTables <- ListTable(readData)
...
tableFill <- function(readData, indIn, data2file){
  ...
  while (checkStr(readData[j])[[1]] == TRUE){
    j <- j + 1
    if( checkStr(readData[j])[[1]] > nElementMax)
