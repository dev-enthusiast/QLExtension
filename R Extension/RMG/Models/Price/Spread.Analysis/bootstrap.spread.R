...
bootstrap.spread <- function(MM, save, options){
  ...
  no.sims         <- 1000   # number of simulations
  days.in.month   <- 30.5
  ...
  for (m in 1:12){
    ...
    spread <- MM[ind,"spread"]
    sim.spread <- sample(spread, hours.in.month*no.sims, replace=TRUE)
    sim.spread <- matrix(sim.spread, ncol=no.sims, nrow=hours.in.month)
    avg.spread.monthly[,m] <- apply(sim.spread, 2, myMean) 
  }
  ...
  caption  <- paste("Bootstraped monthly spread ", options$between, " based on historical data from ", options$dateRange, ".", sep="")
  ...
  mtext("Bootstraped average hourly spread, $ ", side=2, line=2.2)
