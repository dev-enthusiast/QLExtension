...
#
# IMPORTANT: IF YOU CHANGE THIS CODE, RUN THE TESTS BELOW TO MAKE SURE YOU 
# 	DIDN'T BREAK AN EDGE CASE
#
#
cashFlowChange <- function( startExposure=0, endExposure=0, 
...
{   
    ...
    ourSide = NA
    ourSide[ad] = cegThreshold[ad] - startExposure[ad]
    
    adNoCH = which( startingState == .A & endingState == .D & cashHeld == 0 )
    theirSide = NA
    theirSide[adNoCH] = 0
    
    adCH = which( startingState == .A & endingState == .D & cashHeld != 0 )
    theirSide[adCH] = endExposure[adCH] - cptyThreshold[adCH]
      
    cashFlowChange[ad] = ourSide[ad] + theirSide[ad]
