...
#
Weather$readPositionFile <- function( positionFile )
{   
	...
	positionData = data[,c("Location", "Month", "Security.Tenor", "C.P", "Qty", 
					"Strike", "Dollar.Price", "Underlying", "BDelta", "BVega")]
...
{
	quotes = merge( quoteData, 
			unique( positionData[,c("LOCATION_1", "START_DT", "END_DT")]) )
...
Weather$readPositionsFromSecDb <- function( portfolioOrBook, asOfDate=Sys.Date() )
{
	...
	secdb.invoke("_UT Get Portfolio Greeks", "Greeks Rpt::Get Position", 
			portfolioOrBook, asOfDate, asOfDate, 
			as.Date(secdb.dateRuleApply(asOfDate, "+e+1d")), 
			SECDB_END_OF_TIME, FALSE, outputFile )
	
	positionTable = Weather$readPositionFile( outputFile )
...
Weather$getUniqueBooks <- function(date){
	allCurves <- LiveData$allPositions()
	weatherCurves <- allCurves[grep("WTH", allCurves$COMMOD_CURVE),]
	
	uniqueBooks <- levels(factor(weatherCurves$BOOK))
	return(uniqueBooks)
}

##############################################################################################
#function to find weather books (COMMOD_CURVES with "WTH" in name) and then 
#read in each books' positions 
#returns a vector of data frames; each data frame is for a different weather book

Weather$readAllPosFromSecDb <- function(uniqueBooks, date){
...
##############################################################################################
#function to reorganize and aggregate data for positions report
#

Weather$generateReport <- function(){
	
	reportDay <- as.Date(secdb.dateRuleApply(Sys.Date(), "-1b"))
	prevDay <- as.Date(secdb.dateRuleApply(Sys.Date(), "-2b"))
	
	#get a vector of the unique weather book names
	uniqueWeatherBooks <- Weather$getUniqueBooks(reportDay)
	
	#split the books into Portfolio Management books and trading books
	isPMVector <- lapply(uniqueWeatherBooks, BookOwnership$isPM)
	
	#read in all positions for each weather book		
	#allPos <- Weather$readAllPosFromSecDb(reportDay)
	
	#create vector of unique locations 
	#uniqueLocations <- vector()
	#for(bookDataFrame in allPos){
		
	#	levels(factor(allPos[[1]]$LOCATION_1))
	#}
	return(isPMVector)
}
