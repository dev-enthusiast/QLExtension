# Library of xls wrappers.  For price.CDS.R and
#                               calibrate.default.cdf.R
#
#
...
xls.calibrate.default.cdf <- function(quotes, parms){
  require(RODBC) 
  ...
  data$payment.freq <- as.character(parms["payment freq.","Value"])
  data$recovery.rate <- as.numeric(as.character(parms["recovery rate","Value"]))  

  quotes  <- na.omit(data.frame(time=quotes[,1], cds.spread=quotes[,2]))
  ...
  res <- calibrate.default.cdf(quotes, data)
  return(res)
}
