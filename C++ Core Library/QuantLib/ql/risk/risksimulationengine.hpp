...
#ifndef quantlib_risksimulation_engine_hpp
#define quantlib_risksimulation_engine_hpp
...
namespace QuantLib {
    ...
    //! base class for risk Monte Carlo simulation engines
    /*! It accepts an instruments that depends on (Multi)Path.
		and returns the 95% PFE profile, EPE profile etc.
    */
    ...
    class RiskSimulationEngine {
      ...
      protected:
		  RiskSimulationEngine(bool antitheticVariate,
                     bool controlVariate)
	...
	inline typename RiskSimulationEngine<MC, RNG, S>::result_type
		RiskSimulationEngine<MC, RNG, S>::value(Real tolerance,
                                              Size maxSamples,
    ...
    template <template <class> class MC, class RNG, class S>
	inline void RiskSimulationEngine<MC, RNG, S>::calculate(Real requiredTolerance,
                                                  Size requiredSamples,
                                                  Size maxSamples) const {
    ...
    template <template <class> class MC, class RNG, class S>
	inline typename RiskSimulationEngine<MC, RNG, S>::result_type
		RiskSimulationEngine<MC, RNG, S>::errorEstimate() const {
        return mcModel_->sampleAccumulator().errorEstimate();
    }

    template <template <class> class MC, class RNG, class S>
	inline const typename RiskSimulationEngine<MC, RNG, S>::stats_type&
		RiskSimulationEngine<MC, RNG, S>::sampleAccumulator() const {
