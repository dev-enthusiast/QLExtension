...
#ifndef quantlib_stochastic_process_pca_array_hpp
#define quantlib_stochastic_process_pca_array_hpp
...
namespace QuantLib {
	...
	// correlation is reduced to PCA
    /*! \ingroup processes */
    class StochasticProcessPcaArray : public StochasticProcess {
      public:
        StochasticProcessPcaArray(
				  // usually use GeneralizedBlackScholesProcess
                  const std::vector<boost::shared_ptr<StochasticProcess1D> >&,		
                  const Matrix& pcasqrtorrelation);		// e.g., n*3
        // stochastic process interface
        Size size() const;
		Size factorSize() const;
