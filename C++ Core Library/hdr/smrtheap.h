...
#ifdef __MVS__
#ifdef _LONG_LONG
typedef unsigned long long MEM_THREAD_ID;
#else
#define MEM_THREAD_ID unsigned long tid_part_1; unsigned long
#endif /* _LONG_LONG */

#else
typedef unsigned long MEM_THREAD_ID;
#endif
 ...
 */
   ...
   || (defined(__IBMCPP__) && __IBMCPP__ >= 250) \
	...
	|| defined(_AIX43) \
	|| (defined(_SGI_COMPILER_VERSION) && _SGI_COMPILER_VERSION >= 721) \
	|| defined(__linux__) \
	|| (defined(_MSC_VER) && _MSC_VER >= 1210))
...
#if 1 /* !defined(MEM_DEBUG) || !(defined(MEM_WIN16) || defined(MEM_WIN32)) */
#define SHI_MAJOR_VERSION 8
...
#if (UINT_MAX == 0xFFFFu)
typedef unsigned long MEM_SIZET;
#else
typedef size_t MEM_SIZET;
#endif
...
typedef enum
{
   ...
   MEM_NOT_FIXED_SIZE,
   MEM_BAD_FLAGS,
#if defined(MEM_DEBUG) || defined(BUILDING_HA) || defined(MEM_SMP_DEBUG)
   MEM_BAD_BUFFER,
   MEM_DOUBLE_FREE,
   MEM_UNDERWRITE,
   MEM_OVERWRITE, 
...
#endif /* MEM_DEBUG */
	MEM_UNSUPPORTED,
	MEM_EXCEEDED_PROCESS_HEAP_LIMIT,
...
/* all fields below this are valid only for debugging lib                 */
   ...
   MEM_SIZET argSize;    /* size parameter, if applicable             */
   MEM_SIZET argCount;   /* count parameter, if applicable            */
   ...
   MEM_THREAD_ID threadID;    /* ID of thread where error detected */
...
typedef enum
{
   ...
   MEM_EXTERNAL_BLOCK         = 0x0008u,
	MEM_FREE_BLOCK             = 0x0010u, /* non-typed free page */
...
/* Flags passed to MemDefaultPoolThreadExclusive */
typedef enum 
{
	MEM_THREAD_EXCLUSIVE_OFF = 0,        /* OK to free from other threads     */
	MEM_THREAD_EXCLUSIVE_ON  = 0x10000u, /* frees only from allocating thread */
	MEM_FREE_ON_THREAD_TERM  = 0x20000u  /* free pool on thread terminate     */

} MEM_THREAD_EXCLUSIVE_T;
...
#define MEM_ERROR_RET (MEM_SIZET)(-1)
...
#ifdef _SHI_Pool
#define MEM_ENTRY5 MEM_ENTRY1
#else
#define MEM_ENTRY5
#endif /* _SHI_Pool */
MEM_ENTRY5 MEM_POOL MEM_ENTRY MemInitDefaultPool(void);
MEM_ENTRY5 MEM_BOOL MEM_ENTRY MemFreeDefaultPool(void);
#endif /* MEM_MT */
...
/* Process heap usage */
MEM_ENTRY1 MEM_SIZET MEM_ENTRY MemProcessSetGrowIncrement(MEM_SIZET size);
MEM_ENTRY1 MEM_SIZET MEM_ENTRY MemProcessSetFreeBytes(MEM_SIZET size);
MEM_ENTRY1 MEM_SIZET MEM_ENTRY MemProcessSetLargeBlockThreshold(MEM_SIZET size);
MEM_ENTRY1 MEM_SIZET MEM_ENTRY MemProcessSetHeapLimit(MEM_SIZET size);
MEM_ENTRY1 MEM_BOOL MEM_ENTRY MemProcessUseMunmap(MEM_BOOL bMunmap);
MEM_ENTRY1 MEM_BOOL MEM_ENTRY MemProcessCoelesceSystemAllocs(MEM_BOOL bValue);
MEM_ENTRY1 void MEM_ENTRY MemProcessFlushAll(void);

#if defined(_AIX) || defined(WIN32)
MEM_ENTRY1 MEM_BOOL MEM_ENTRY MemPatchProcess(void);
#endif /* _AIX */

/* Memory Pool Functions */
_shAPI(MEM_POOL, MemPoolInit)(unsigned _dbgARGS);
_shAPI(MEM_POOL, MemPoolInitFS)(MEM_USHORT, MEM_SIZET, 
   unsigned _dbgARGS);
_shAPI(MEM_POOL, MemPoolInitRegion)(void MEM_FAR *,
												MEM_SIZET size, unsigned _dbgARGS);
_shAPI(MEM_POOL, MemPoolInitRegionEx)(void MEM_FAR *addr,
	MEM_SIZET size, unsigned flags, void MEM_FAR *security _dbgARGS);
...
_shAPI(MEM_POOL, MemPoolInitNamedSharedEx)(void MEM_FAR *addr,
   ...
   const char MEM_FAR *name, MEM_SIZET size, unsigned flags _dbgARGS);
...
MEM_ENTRY1 unsigned MEM_ENTRY MemPoolSetMaxSubAlloc(MEM_POOL, unsigned);
MEM_ENTRY1 MEM_BOOL MEM_ENTRY MemPoolSetPageResizing(MEM_POOL, MEM_BOOL);
_shAPI(MEM_BOOL, MemPoolSetBlockSizeFS)(MEM_POOL, MEM_USHORT _dbgARGS);
MEM_ENTRY1 MEM_SIZET MEM_ENTRY MemPoolSetGrowIncrement(MEM_POOL,
	MEM_SIZET);
...
#define MemPoolSetHighThreads(p, b)
MEM_ENTRY1 MEM_BOOL MEM_ENTRY MemPoolSetMaxSubpools(MEM_POOL, unsigned);
MEM_ENTRY1 MEM_BOOL MEM_ENTRY MemPoolSetSerialization(MEM_POOL, MEM_BOOL);
MEM_ENTRY1 MEM_BOOL MEM_ENTRY MemDefaultPoolThreadExclusive(unsigned flags);
_shAPI(MEM_BOOL, MemPoolSetSmallBlockSize)(MEM_POOL, MEM_USHORT _dbgARGS);
_shAPI(MEM_SIZET, MemPoolSetFloor)(MEM_POOL, MEM_SIZET _dbgARGS);
_shAPI(MEM_SIZET, MemPoolSetCeiling)(MEM_POOL, MEM_SIZET _dbgARGS);
_shAPI(MEM_SIZET, MemPoolPreAllocate)(MEM_POOL, MEM_SIZET, 
   MEM_BLOCK_TYPE _dbgARGS);
_shAPI(MEM_SIZET, MemPoolPreAllocateHandles)(MEM_POOL,
   MEM_SIZET _dbgARGS);
_shAPI(MEM_SIZET, MemPoolShrink)(MEM_POOL _dbgARGS);
_shAPI(MEM_SIZET, MemPoolSize)(MEM_POOL _dbgARGS);
_shAPI(MEM_SIZET, MemPoolCount)(MEM_POOL _dbgARGS);
...
_shAPI(MEM_HANDLE, MemAlloc)(MEM_POOL, unsigned, MEM_SIZET _dbgARGS);
_shAPI(MEM_HANDLE, MemReAlloc)(MEM_HANDLE,MEM_SIZET,unsigned _dbgARGS);
...
_shAPI(MEM_SIZET, MemSize)(MEM_HANDLE _dbgARGS);
_shAPI(MEM_SIZET, MemSizeRequested)(MEM_HANDLE _dbgARGS);
...
_shAPI(void MEM_FAR*,MemAllocPtr)(MEM_POOL,MEM_SIZET,unsigned _dbgARGS);
_shAPI(void MEM_FAR *, MemReAllocPtr)(void MEM_FAR *, MEM_SIZET,
   unsigned _dbgARGS);
_shAPI(MEM_BOOL, MemFreePtr)(void MEM_FAR * _dbgARGS);
_shAPI(MEM_SIZET, MemSizePtr)(void MEM_FAR * _dbgARGS);
_shAPI(MEM_POINTER_STATUS, MemCheckPtr)(MEM_POOL, void MEM_FAR * _dbgARGS);
MEM_ENTRY1 void MEM_FAR * MEM_ENTRY MemAllocAligned(MEM_POOL, MEM_SIZET,
	MEM_SIZET, unsigned);
MEM_ENTRY1 void MEM_FAR * MEM_ENTRY MemAllocAlignedOffset(MEM_POOL, MEM_SIZET,
	MEM_SIZET, MEM_SIZET, unsigned);
...
MEM_BOOL shi_call_malloc_new_handler_msc(void);
