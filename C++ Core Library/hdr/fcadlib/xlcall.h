/****************************************************************
**
**	fcadlib/xlcall.h - From Microsoft with minor changes:
**		Wrap in #ifndef
**		Some extra definitions at top
**
**	$Log: xlcall.h,v $
**	Revision 1.4  2012/03/02 16:22:21  e19351
**	Export Excel12 functions for other libs (ex. exslang).
**	AWR: #258495
**
**	Revision 1.3  2012/03/01 20:14:29  e19351
**	Updated headers to Excel12 API, added ExcelValue wrapper class to use it.
**	Improved resize logic. Check slang arg datatype before converting to time.
**	Wrapped new changes in env variables to phase in.
**
**	Revision 1.2  2001/03/13 13:03:13  goodfj
**	Adjust includes, in an attempt to reduce dependencies in wrapper projs
**
**	Revision 1.1  1998/08/17 03:46:48  gribbg
**	Initial version
**	
**
****************************************************************/

#ifndef IN_FCADLIB_XLCALL_H
#define IN_FCADLIB_XLCALL_H

#include	<portable.h>
#include	<windows.h>
#include	<fcadlib/base.h>

// Make sure we export this stuff from XLCALL.cpp.
struct xloper12;
EXPORT_C_FCADLIBX int _cdecl Excel12(int xlfn, xloper12* operRes, int count,... );
EXPORT_C_FCADLIBX int pascal Excel12v(int xlfn, xloper12* operRes, int count, xloper12* opers[]);

// VC6 doesn't have this..
...
// Some extra stuff that should be included in the toolkit, but isn't.
#define xlGetCellArgRef				1 // Absolute-style reference of the top left cell in reference as text in the [Book1.xls]Sheet1!$A$1 style.
#define xlGetCellArgVal				5 // The value of the top left cell.
#define xlGetCellArgFormula			6 // The formula in the top left cell in A1 or R1C1 style as determined by workspace settings.
#define xlGetCellArgFormat			7 // The number format of the top left cell.
#define xlGetCellArgLocked			14 // Returns true if the top left cell is locked.
#define xlGetCellArgHidden			15 // Returns true if the top left cell’s formula is hidden.
#define xlGetCellArgWidth			16 // Returns 2-column row vector: 1st column: Width of the left-most column 2nd column: True if the width is the standard width, false if a custom width has been set.
#define xlGetCellArgHeight			17 // Height of top row in points.
#define xlGetCellArgName			32 // The name of the workbook and sheet containing the reference in the form [Book1.xls]Sheet1, unless the window contains only a single sheet that has the same name as the workbook without its extension, in which case the form BOOK1.XLS.
#define xlGetCellArgRawFormula		41 // Returns the formula in the active cell without translation into the language set for the workspace.
#define xlGetCellArgHasNote			46 // True if the top left cell has a text note.
#define xlGetCellArgHasFormula		48 // True if the top left cell contains a formula, false if constant.
#define xlGetCellArgHasArrayFormula 49 // True if the cell is part of an array formula.
#define xlGetCellArgString			52 // If the top left cell is a string constant, the text alignment character (`), otherwise empty text ("").
#define xlGetCellArgText			53 // The top left cell as displayed, converted to text, including formatting numbers and symbols.
#define xlGetCellArgSheetName		62 // The name of the workbook and the current sheet in the form [Book1.xls]Sheet1.
#define xlGetCellArgWorkbookName	66 // The workbook name containing the range in the form Book1.xls.
