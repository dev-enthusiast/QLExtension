/***********************************************************//**
**
** $Header: /home/cvs/src/rglue/src/sdbInterface.h,v 1.2 2012/07/27 13:13:46 khodod Exp $
**
** @file sdbInterface.h
**
...
public:
	...
	/** Get connection to given database, database nami is optional, and if 
    ommitted it is deduced from evironment variable */
	EXPORT_CPP_SDBINTERFACE static SecDbInterface * instance (
			const char * dbName = 0
			); 

	/** Set Tracer -- object which will print out Slang output as well as some 
    other messages, by default all the output goes to nul. Unfortunetly there 
    is only one tracer object per application, so there is no way to print out
	trace in different ways for different instances of SecDbInterface. This 
    limitation dictated by underlying SecDb C API.
    */
	...
	/** Evaluate given slang code. Note: slangScript is a code not 
        a script name! */
	virtual dt_value_var evalSlang (const char * slangScript) = 0;

	/** Load Slang library and return SlangInvoker interface capable of 
        invoking functions from the library */
	virtual SlangInvoker_ptr getInvoker (const char * scriptName) = 0;

	/** Start Transaction. Returns transaction object which by default 
        rollbacks transaction in dtor, unless being explicetly asked to 
        commit it. */
...
/*****************************************************************************/
/** SlangInvoker provides ability to invoke functions defined in 
    slang script (library). */
class SlangInvoker
{
public:
...
/** SecDb Transaction object.
  ...
  * During object construction Transaction started, unless being explicetly 
    commited the transaction will be rolled back in destructor
  */
class SecDbTransaction
{
public:
	/** Don't be fooled with empty body, it is an interface, actual 
        implementation will roll transaction back. */
...
/*****************************************************************************/
/** Tracer interface. 
  * SecDbInterface itself, as well as Slang scripts ans function could print 
    something out, since particular applications might want to print it in 
    different ways (to stdout, to the file, ignore, to some proprietary 
    interface such as mexPrintf for Matlab etc) actual printing is a 
    responsibility of the client. Client either reuses one of existing 
    implementations of the SecDbTracer interface, or creates his own 
    implementation. Implementation passed then to SecDbInterface.
  */
class SecDbTracer
{
public:
	...
	virtual void vtrace (const char * msg, va_list argp) const = 0;
