#define GSCVSID "$Header: /home/cvs/src/rglue/src/Rcpp.cpp,v 1.2 2012/07/27 13:13:46 khodod Exp $"
...
SEXP RcppFrame::getReturnValue() {
    int numProtected = 0;

	vector<string> columnNames = getColNames();
	int ncol = columnNames.size();
    
	vector<vector<ColDatum> > table = getTableData();
	int nrow = table.size();
    ...
    for(int i=0; i < ncol; i++) {
		...
		else if(table[0][i].getType() == COLTYPE_TIME) {
			value = PROTECT(allocVector(REALSXP,nrow));
			numProtected++;
			for(int j=0; j < nrow; j++){
				REAL(value)[j] = table[j][i].getTimeValue();
			}
			SEXP timeclass = PROTECT(allocVector(STRSXP,1));
			numProtected++;
			SET_STRING_ELT(timeclass, 0, mkChar("POSIXct"));
			setAttrib(value, R_ClassSymbol, timeclass); 
		}
		else {
	...
	SEXP dataframeClass = PROTECT(allocVector(STRSXP,1));
	numProtected++;
	SET_STRING_ELT(dataframeClass, 0, mkChar("data.frame"));
	setAttrib(rl, R_ClassSymbol, dataframeClass);*/
