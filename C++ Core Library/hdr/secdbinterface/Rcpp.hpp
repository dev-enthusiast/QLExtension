...
enum ColType { COLTYPE_DOUBLE, COLTYPE_INT, COLTYPE_STRING,
	       COLTYPE_FACTOR, COLTYPE_LOGICAL, COLTYPE_DATE, COLTYPE_TIME };
...
public:
	...
	void setTimeValue(double time) { x = time, type = COLTYPE_TIME; }
	...
	double getTimeValue() { 
	if(type != COLTYPE_TIME)
	    throw std::range_error("RcppFrame: wrong data type in getTimeValue");
	return x; 
    }
